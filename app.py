...
# --- BACKEND PIPELINE (EXTREME SPEED OPTIMIZATION) ---
...
def run_pipeline(file_bytes):
    ...
    # Models (SUPER LIGHTWEIGHT: n_estimators=30, max_depth limited, n_jobs=1 on BOTH)
    rf = RandomForestClassifier(n_estimators=30, max_depth=10, random_state=42, n_jobs=1)
    rf.fit(X_train, y_train)
    acc = accuracy_score(y_test, rf.predict(X_test))
    
    reg = XGBRegressor(n_estimators=30, max_depth=6, random_state=42, n_jobs=1)
...
if "ctx" not in st.session_state or st.session_state.get("fid") != fid:
    with st.spinner("⚙️ Fast-booting Analysis Engines (ETA: 5 seconds)..."):
